...
### Added
- Serbian (`sr`) and Macedonian (`mk`) widget locales

### Changed
- Shortened the Czech empty-state copy

## [0.3.2] - 2026-09-25

### Added
- Boot `theme` (`light` | `dark` | `auto`) applied on the shadow host, including `:host(.dark)` tokens and `prefers-color-scheme` for `auto`
- `X-Cusu-Sdk-Version` on widget API requests (boot, identify, realtime session, transcribe)

## [0.3.1] - 2026-09-25

### Added
- Boot `rating_scale: "off"` hides the post-resolution rating prompt

## [0.3.0] - 2026-09-24

### Added
- Script-tag install from the Zerops object-storage CDN (`widget/<version>/cusu.js`, plus `widget/latest/cusu.js`)
- Release workflow uploads the IIFE before `npm publish` and purges `latest`

## [0.2.1] - 2026-09-24

### Fixed
- When boot `reply_locale` is `auto`, widget chrome prefers page language (`document.documentElement.lang`) or `config.locale` over the group language
